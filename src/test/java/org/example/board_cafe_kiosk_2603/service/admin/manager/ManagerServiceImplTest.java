...
class ManagerServiceImplTest {
    ...
    void CreateAndFindAll() {
        ...
        log.info("--- 전체 목록 조회 완료 (사이즈: {}) ---", list.size());
    ...
    void updateActive() {
        ...
        // 2. 등록된 녀석의 실제 ID(PK) 가져오기
        List<ManagerResponse> list = managerService.findAll();
        ManagerResponse target = list.get(0); // 가장 최근 혹은 첫번째 데이터
        ...
        // 4. 검증: 다시 조회해서 상태 확인
        List<ManagerResponse> updatedList = managerService.findAll();
        ManagerResponse updated = updatedList.stream()
